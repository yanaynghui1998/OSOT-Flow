...
class Runner(object):
    ...
    def compute_label_flow(self, step, x0, xt):
        """ Eq 12 """
        # std_fwd = self.diffusion.get_std_fwd(step, xdim=x0.shape[1:])
        label = (xt - x0) / step
        return label.detach()
    ...
    def train(self, opt, train_dataset, val_dataset, corrupt_method):
        ...
        for it in range(opt.num_itr):
            ...
            for _ in range(n_inner_loop):
                ...
                x0, x1, mask, y, cond = self.sample_batch(opt, train_loader, corrupt_method)#x0 is the clean image, x1 is the LR image
    ...
    def ddpm_dps_sampling(self, opt, x1, mask=None, cond=None, clip_denoise=False, nfe=None, log_count=10, verbose=True):
        ...
        with self.ema.average_parameters():
            ...
            xs, pred_x0 = self.diffusion.ddpm_dps_patch_sampling(
                steps, pred_x0_fn, x1, mask=mask, ot_ode=opt.ot_ode, log_steps=log_steps, verbose=verbose,
            )
    ...
    def evaluation(self, opt, it, val_loader, corrupt_method):
        ...
        pred_x0s    = all_cat_cpu(opt, log, pred_x0s)#(1,10,1,32,32,32)

        batch, len_t, *xdim = xs.shape
        _,_,*xdim_2D=xs[:,:,:,:,8,:].shape
        assert img_clean.shape == img_corrupt.shape == (batch, *xdim)
        assert xs.shape == pred_x0s.shape
        assert y.shape == pred_x0s[:,0,:,:,:].shape
        ...
        log_image("debug/pred_clean_traj", pred_x0s[:,:,:,:,16,:].reshape(-1, *xdim_2D), nrow=len_t)
        log_image("debug/recon_traj",      xs[:,:,:,:,16,:].reshape(-1, *xdim_2D),      nrow=len_t)

        log.info(f"========== Evaluation finished: iter={it} ==========")
        torch.cuda.empty_cache()

    @torch.no_grad()
    def real2complex(self,data):
        if len(data.shape)==5:
            data_real=data[:,0:1,:,:,:]
            data_img=data[:,1:2,:,:,:]
        else:
        ...
        data_complex=data_real+1j*data_img
        data_abs=torch.abs(data_complex)
        return data_abs

    @torch.no_grad()
    def complex_evaluation(self, opt, it, val_loader, corrupt_method):
        ...
        #convert two channel images to single channel abs images 
        img_clean=self.real2complex(img_clean)
        img_corrupt=self.real2complex(img_corrupt)
        y=self.real2complex(y)
        xs=self.real2complex(xs)
        pred_x0s=self.real2complex(pred_x0s)
        ...
        y           = all_cat_cpu(opt, log, y)
        xs          = all_cat_cpu(opt, log, xs)
        pred_x0s    = all_cat_cpu(opt, log, pred_x0s)#(1,10,2,32,32,32)

        batch, len_t, *xdim = xs.shape
        _,_,*xdim_2D=xs[:,:,0:1,:,8,:].shape
        assert img_clean.shape == img_corrupt.shape == (batch, *xdim)
        assert xs.shape == pred_x0s.shape
        assert y.shape == pred_x0s[:,0,:,:,:].shape
        log.info(f"Generated recon trajectories: size={xs.shape}")
        ...
        log.info("Logging images ...")
        img_recon = xs[:, 0, ...]
        log_image("image/clean",   img_clean[:,:,:,32,:])
        log_image("image/corrupt", img_corrupt[:,:,:,32,:])
        log_image("image/recon",   img_recon[:,:,:,32,:])
        log_image("debug/pred_clean_traj", pred_x0s[:,:,:,:,32,:].reshape(-1, *xdim_2D), nrow=len_t)
        log_image("debug/recon_traj",      xs[:,:,:,:,32,:].reshape(-1, *xdim_2D),      nrow=len_t)
