...
class SRConv(H_functions):
    def mat_by_img(self, M, v, dim):
        return torch.matmul(M, v.reshape(v.shape[0] * self.channels, dim,
                        dim, dim)).reshape(v.shape[0], self.channels, M.shape[0], dim,dim)

    def img_by_mat(self, v, M, dim):
        return torch.matmul(v.reshape(v.shape[0] * self.channels, dim,
                        dim,dim), M).reshape(v.shape[0], self.channels, dim, dim, M.shape[1])
    ...
    def U(self, vec):
        #invert the permutation
        temp = torch.zeros(vec.shape[0], self.small_dim**3, self.channels, device=vec.device)
        temp[:, :self.small_dim**3, :] = vec.clone().reshape(vec.shape[0], self.small_dim**3, self.channels)
        temp = temp.permute(0, 2, 1)
...
def build_sr_bicubic(factor, device, image_size, data_channels=1):
...
def build_sr4x(opt, log, sr_filter, image_size):
    ...
    def sr4x(img):
        ...
        if sr_filter == "pool":
            _img = sr_pool.H(img).reshape(b, c, w//factor , h//factor , d//factor)
        elif sr_filter == "bicubic":
            _img = sr_bicubic.H(img).reshape(b, c, w//factor , h//factor , d//factor)
